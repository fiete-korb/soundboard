...
function editUser($id,$email,$firstname,$lastname,$dialing_code="0",$phone="0",$city="Berlin",$country="DE")
{
		    $id = secureInput($id);
		 $email = "0";
...
function getAllUsers()
{
	...
	while ($a_row = mysql_fetch_array($res))
	{
		$allUser[$c]["id"] = $a_row["id"];
		$allUser[$c]["username"] = $a_row["username"];
		$allUser[$c]["first_name"] = $a_row["first_name"];
		$allUser[$c]["last_name"] = $a_row["last_name"];
		$allUser[$c]["email"] = $a_row["email"];
		$allUser[$c]["dialing_code"] = $a_row["dialing_code"];
		$allUser[$c]["phone"] = $a_row["phone"];
		$allUser[$c]["city"] = $a_row["city"];
		$allUser[$c]["country"] = $a_row["country"];
		$allUser[$c]["thumb_path"] = $a_row["thumb_path"];
		$allUser[$c]["img_path"] = $a_row["img_path"];
		$allUser[$c]["active"] = $a_row["active"];
		$allUser[$c]["reg_date"] = $a_row["reg_date"];
		$allUser[$c]["last_active"] = $a_row["last_active"];
...
function confirm_user_reg($prsn)
{
	...
	$query = mysql_query("SELECT username,active FROM users WHERE username = '".$prsn."'");
	...
	if(mysql_num_rows($query)==1)
	{
		...
		if($row['active']==0)
		{
			$update = mysql_query("UPDATE users SET active=1 WHERE username = '".$prsn."'");
			if($update){
				return 99;
			} else return 1;
		}
...
function displayUserImg($id, $height = 200)
{
	...
	if (!empty($row['thumb_path'])){
		echo "<img src='".$row['thumb_path']."' width='".$height."' border='0' alt='' hspace='2' />";
	} else {
		//display a default image if user image does not exist
		echo "<img src='pics/no_image.gif' width='".$height."' border='0' alt='' hspace='2' />";
	}
}
